  ...
  return (
    <section className='sidebar-section' style={{ maxHeight: '50vh', borderLeft: `4px solid ${layers['addresses'].color}` }}>
      <div className="flex items-center justify-between">
        ...
        <FontAwesomeIcon icon={open ? faChevronCircleDown : faChevronCircleRight} onClick={() => setOpen(!open)} className="text-2xl mr-2 mt-1" />
      </div>
      <AnimateHeight
        duration={250}
        height={open ? 'auto' : 0}>

        <div className="overflow-y-auto text-sm pr-3 mt-3" style={{ maxHeight: '45vh' }}>
          {addresses.map((a, i) => (
