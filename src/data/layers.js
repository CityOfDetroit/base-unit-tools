    ...
    formatter: (p) => {
      return {
        ...
        attributes: {
          ...
          "Taxpayer address": `${p.properties.taxpayer_address}, ${p.properties.taxpayer_city}, ${p.properties.taxpayer_state} ${p.properties.taxpayer_zip_code}`,
          "Sale date": p.properties.sale_date ? moment(p.properties.sale_date).format("LL") : null,
          "Sale price": `$${parseInt(p.properties.amt_sale_price).toLocaleString()}`,
          "PRE %": p.properties.pct_pre_claimed.toLocaleString(),
          "NEZ": p.properties.nez,
          "Is Improved?": p.properties.is_improved === 0 ? "No" : "Yes",
          "Property Class": `${p.properties.property_class} - ${p.properties.property_class_description}`,
          "Property Use": `${p.properties.use_code} - ${p.properties.use_code_description}`,
          "Total Floor Area": p.properties.total_floor_area ? parseInt(p.properties.total_floor_area).toLocaleString() : null,
          "Style": p.properties.building_style,
          "Taxable status": p.properties.tax_status_description,
          "Assessed value": parseInt(p.properties.amt_assessed_value).toLocaleString(),
          "Taxable value": parseInt(p.properties.amt_taxable_value).toLocaleString(),
          ...
          "Zoning": p.properties.zoning_district,
          "Historic Designation": p.properties.local_historic_district
        },
        groups: {
          "Ownership": [
            "Taxpayer",
